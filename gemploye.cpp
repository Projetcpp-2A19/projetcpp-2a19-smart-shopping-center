...
void gemploye::on_pushButton_stats_clicked()
{
    ...
    public:
        PieWidget(QMap<QString, int> data, QWidget *parent = nullptr)
            : QWidget(parent), statsMap(data)
        {
            // Génération de couleurs entre rose et bleu
            QList<QColor> baseColors = {
                QColor("#e91e63"),  // rose
                QColor("#2196f3"),  // bleu
                QColor("#f06292"),  // rose clair
                QColor("#42a5f5"),  // bleu clair
                QColor("#ce93d8"),  // violet rosé
                QColor("#64b5f6")   // bleu doux
            };

            int i = 0;
            for (auto it = statsMap.begin(); it != statsMap.end(); ++it, ++i)
                colors.append(baseColors[i % baseColors.size()]);
        }
    ...
    protected:
        void paintEvent(QPaintEvent *) override {
            ...
            for (auto it = statsMap.begin(); it != statsMap.end(); ++it, ++index) {
                ...
                painter.drawPie(rect, int(startAngle * 16), int(angleSpan * 16));
                ...
                double cy = rect.center().y() - radius * qSin(radians);  // -sin car Qt a un Y inversé
...
void gemploye::on_pushButton_Locataires_clicked()
{
    GLocataire *locataireWindow = new GLocataire(this); // `this` pour que la fenêtre soit liée à la fenêtre principale
    locataireWindow->show(); // Ouvre comme une boîte de dialogue modale
    // locataireWindow->show(); // Alternative : fenêtre non-modale
}


void gemploye::on_pushButton_Evennements_clicked()
{
