...
#include <QMessageBox>
#include <QMessageBox>       // Pour afficher les boîtes de dialogue d'alerte et de confirmation
...
#include <QSqlQuery>
#include <QFileDialog>
#include<QPdfWriter>// Ajouter cette ligne pour inclure QFileDialog
// Pour afficher un message en cas d'erreur de chargement d'image
#include <QInputDialog>
#include <QBuffer>  // Ajouter cette ligne pour déclarer QBuffer
#include <QImage>
...
void GEvennement::on_pushButton_Modifier_clicked()
{
    ...
    // Vérifier si l'employé existe et récupérer ses données actuelles
    QSqlQuery query;
    ...
    // Création d'une boîte de dialogue pour modifier les valeurs
    QDialog dialog(this);
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        salaire = lineSalaire.text().toInt();
        telephone = lineTelephone.text().toInt();
        ...
        if (updateQuery.exec()) {
            ...
            afficher(); // Met à jour l'affichage des employés
        } else {
...
void GEvennement::on_pushButton_Extraire_clicked()
{
    QString id = ui->lineEdit_id_2->text().trimmed();
...
void GEvennement::on_pushButton_rechercher_clicked()
{
    ...
    // Si l'employé est trouvé, afficher les résultats dans un tableau ou une autre interface
    ...
    // Affichage des informations dans un tableau (tableWidget) ou autre widget
    ui->tableWidget->setRowCount(0); // Réinitialiser le tableau avant d'afficher les résultats
    ui->tableWidget->insertRow(0); // Insérer une nouvelle ligne pour l'employé trouvé
...
void GEvennement::on_pushButton_trier_clicked()
{
    QSqlQuery query;
