...
void GLocataire::populateFieldsFromSelection(const QModelIndex &index)
{
    ...
    ui->line_TYPELoc->setText(model->data(model->index(row, 6)).toString());
    ...
    ui->line_CONTRATLoc->setText(model->data(model->index(row, 3)).toString());
...
void GLocataire::on_pushButton_deconnecter_clicked()
{
    // Déconnexion
